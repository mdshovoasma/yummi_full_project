...
// print_r($banner_img);
 ...
 if($banner_img['size']==0){
    ...
    $updated_query =" UPDATE master_chif SET title='$title',details='$details',chif_category='$chif_category' WHERE id = '$id'";
...
// exit();
    ...
    if(!in_array($extension,$excepted_img)){
        ...
        $chack_master_img_server =" SELECT  banner_img  FROM master_chif WHERE id = '$id'";

        $query_ex =mysqli_query($conn,$chack_master_img_server);
        $server_img = mysqli_fetch_assoc($query_ex);

        // print_r($server_img);
        if(file_exists("../uploads/chif_img/".$server_img['banner_img']) ){

          unlink("../uploads/chif_img/".$server_img['banner_img']);
        ...
        $master_update_width_img = "UPDATE master_chif SET title='$title',details='$details',chif_category='$chif_category',banner_img='$unic_name' WHERE id = '$id'";
